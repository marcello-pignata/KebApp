...
public class MainActivity extends AppCompatActivity
{
    ...
    public void RefreshOrdini(View currentView, ArrayList<Ordine> ordini, boolean showAssegnaATeStesso)
    {
        ...
        for (int i=0; i < ordini.size(); i++)
        {
            ...
            newCard.setBackgroundColor(ContextCompat.getColor(this, android.R.color.transparent));
            ...
            newCard.setOnClickListener(item ->
            {
                if(isNetworkAvailable())
                {
                    ...
                    View popupViewMain;
                    if (utente.get(0).fattorino && showAssegnaATeStesso) {
                ...
                {
                    Toast.makeText(this, "Connessione ad internet non disponibile, impossibile accedere all'ordine",Toast.LENGTH_SHORT).show();
                }
            });
        ...
        executor.execute(() ->
        {
            for (Map.Entry<Ordine, ImageView> set : riferimentiPerAPI.entrySet())
            {
                ...
                try {
                    ...
                    Log.d(TAG, set.getKey().indirizzo + " --> " + prec);


                    Log.d(TAG, "set.getKey().pioggia = " + set.getKey().pioggia);
                    Log.d(TAG, "set.getKey().status = " + set.getKey().status);
                    if (prec >= 0.1)
                    {
